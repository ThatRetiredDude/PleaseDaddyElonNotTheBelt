...
# Repo root on sys.path (sibling: xeraser_analytics.py)
...
from pde.constants import (
    ...
    AI_MODELS,
    AI_REQUEST_USER_AGENT,
    CHUNK_SIZE,
)
from pde.paths import (
    CREDENTIALS_FILE,
    HISTORY_FILE,
    REPO_ROOT,
    TOS_LAST_RUN_FILE,
...
if not _ai_logger.handlers:
    _fh = logging.FileHandler(
        os.path.join(REPO_ROOT, "ai_requests.log"), encoding="utf-8"
    )
...
class XBulkDeleter:
    def __init__(self, *, run_event_loop: bool = True, headless: bool = False):
        self.root = tk.Tk()
...
• my_tweets.json, deleted_history.json, and tos data use rotating .bak saves so a crash is less likely to leave half-written JSON."""
    ...
    def setup_auth_tab(self, parent):
        ...
        self.cred_keyring_var = tk.BooleanVar(value=secure_creds.is_keyring_available())
        ttk.Checkbutton(
            cred_frame,
            text="Store in OS keychain (recommended; otherwise plain x_credentials.json next to the app).",
            variable=self.cred_keyring_var,
        ).grid(row=6, column=0, columnspan=2, sticky="w", pady=(2, 8), padx=(0, 0))
    ...
    def load_credentials(self):
        creds, source = secure_creds.load_credential_map(CREDENTIALS_FILE)
        if creds:
            self.cred_keyring_var.set(source == "keyring")
        else:
            self.cred_keyring_var.set(secure_creds.is_keyring_available())
        if not creds:
            return
        for var, key in zip(
            self.cred_vars,
    ...
    def save_credentials(self):
        ...
        where = secure_creds.save_credential_map(
            CREDENTIALS_FILE, creds, use_keyring=self.cred_keyring_var.get()
        )
        if where == "keyring":
            messagebox.showinfo("Saved", "Credentials saved in the OS keychain. Plain x_credentials.json removed if present.")
        else:
            messagebox.showinfo("Saved", f"Credentials saved to {os.path.basename(CREDENTIALS_FILE)}.")
        self.init_client()
    ...
    def test_auth(self):
        ...
        try:
            ...
            self._update_x_account_display()
            stored, _ = secure_creds.load_credential_map(CREDENTIALS_FILE)
            if stored is not None:
                try:
                    secure_creds.update_stored_x_username(
                        CREDENTIALS_FILE,
                        username,
                        use_keyring=self.cred_keyring_var.get(),
                    )
